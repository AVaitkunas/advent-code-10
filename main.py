...
RULES = {
    north: {"7": west, "|": north, "F": east},
    south: {"|": south, "L": east, "J": west},
    west: {"L": north, "-": west, "F": south},
    east: {"-": east, "7": south, "J": north},
}

input_ = """F--FJ77F-L7---J-F777-F7|77F|-J-F|77|F-L-J-77-L7FL77-7|----.F7-|-|.7-.-FL-FF-FF77.|.F-F-.F|.7.F.7-F-77.J7|----7LL7F|7|-F-J-777-77.F-7-FL77FF|
...
def coordinates_and_values(input_2d: List[List[str]]) -> Tuple[int, int, str]:
    for y, row in enumerate(input_2d):
        for x, value in enumerate(row):
            yield x, y, value


def start_coordinate(input_2d: List[List[str]]) -> Optional[Tuple[int, int]]:
    for x, y, val in coordinates_and_values(input_2d):
        if val == "S":
            return x, y


def walk_while(input_2d: List[List[str]], xx: int, yy: int, dir_, depth):
    while True:
        # path.append((xx, yy))
        # path2[yy][xx] = input_2d[yy][xx]
        rules = RULES[dir_]
...
if __name__ == '__main__':
    d2 = [list(line) for line in input_.splitlines() if line]
    x_start, y_start = start_coordinate(d2)
    start_dir = west
    # path2 = [["."] * len(d2[0]) for _ in range(len(d2))]
    # path = []
    a = walk_while(d2, x_start, y_start, start_dir, 0)
    print(round(a / 2))
    ...
    #
    #
    # p = matplotlib.path.Path(path)
    # c = 0
    # for x_, y_, v in coordinates_and_values(d2):
    #     if (x_, y_) in path:
    #         continue
    #     if p.contains_point((x_, y_)):
    #         c += 1
    # print(c)
    #
    # sum_ = 0
    ...
    #     line_sum = 0
    #     for x, value in enumerate(row):
    #         if value in ("7", "|", "F", "L", "J") and (x, y) in path:
    #             c_ += 1
    #         if value == "." and c_ % 2 == 1:
    #             line_sum += 1
    #     # print(y, line_sum)
